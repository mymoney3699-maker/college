// ============================================================
// grade_appeals.js v1.0.5 - إدارة طعون النتائج وتعديل الدرجات
// ============================================================
...
// ============================================================
// دالة ذكية لتحديد نوع الطعن والدرجات السابقة والجديدة والسبب
// ============================================================
function getAppealGradeDetails(app) {
    const isMidterm = app.appeal_type === 'midterm' || (app.appeal_type_display && app.appeal_type_display.includes('نصفي'));
    const isFinal   = app.appeal_type === 'final'   || (app.appeal_type_display && app.appeal_type_display.includes('نهائي'));

    let gradeTypeLabel = 'الامتحان';
    let gradeBefore = '0';
    let gradeAfter  = '—';

    if (isMidterm) {
        gradeTypeLabel = 'النصفي';
        
        // 1. الدرجة قبل الطعن (الأولوية لـ old_midterm_grade القادمة من الباك إند)
        if (app.old_midterm_grade !== null && app.old_midterm_grade !== undefined) {
            gradeBefore = app.old_midterm_grade;
        } else if (app.previous_midterm_grade !== null && app.previous_midterm_grade !== undefined) {
            gradeBefore = app.previous_midterm_grade;
        } else if (app.prev_midterm_grade !== null && app.prev_midterm_grade !== undefined) {
            gradeBefore = app.prev_midterm_grade;
        } else if (app.status !== 'completed' && app.current_midterm_grade !== null && app.current_midterm_grade !== undefined) {
            gradeBefore = app.current_midterm_grade;
        } else {
            gradeBefore = 0;
        }

        // 2. الدرجة بعد الطعن (الأولوية لـ new_midterm_grade)
        if (app.status === 'completed') {
            if (app.new_midterm_grade !== null && app.new_midterm_grade !== undefined) {
                gradeAfter = app.new_midterm_grade;
            } else if (app.current_midterm_grade !== null && app.current_midterm_grade !== undefined) {
                gradeAfter = app.current_midterm_grade;
            } else {
                gradeAfter = gradeBefore;
            }
        }
    } else if (isFinal) {
        gradeTypeLabel = 'النهائي';
        
        // 1. الدرجة قبل الطعن (الأولوية لـ old_final_grade)
        if (app.old_final_grade !== null && app.old_final_grade !== undefined) {
            gradeBefore = app.old_final_grade;
        } else if (app.previous_final_grade !== null && app.previous_final_grade !== undefined) {
            gradeBefore = app.previous_final_grade;
        } else if (app.prev_final_grade !== null && app.prev_final_grade !== undefined) {
            gradeBefore = app.prev_final_grade;
        } else if (app.status !== 'completed' && app.current_final_grade !== null && app.current_final_grade !== undefined) {
            gradeBefore = app.current_final_grade;
        } else {
            gradeBefore = 0;
        }
        ...
        // 2. الدرجة بعد الطعن
        if (app.status === 'completed') {
            if (app.new_final_grade !== null && app.new_final_grade !== undefined) {
                gradeAfter = app.new_final_grade;
            } else if (app.current_final_grade !== null && app.current_final_grade !== undefined) {
                gradeAfter = app.current_final_grade;
            } else {
                gradeAfter = gradeBefore;
            }
        }
    } else {
        gradeTypeLabel = 'الدرجة';
        const bMid = app.old_midterm_grade ?? app.previous_midterm_grade ?? (app.status !== 'completed' ? app.current_midterm_grade : 0);
        const bFin = app.old_final_grade   ?? app.previous_final_grade   ?? (app.status !== 'completed' ? app.current_final_grade : 0);
        gradeBefore = `نصفي: ${bMid} | نهائي: ${bFin}`;

        if (app.status === 'completed') {
            const aMid = app.new_midterm_grade ?? app.current_midterm_grade ?? bMid;
            const aFin = app.new_final_grade   ?? app.current_final_grade   ?? bFin;
            gradeAfter = `نصفي: ${aMid} | نهائي: ${aFin}`;
        }
    }

    // استخراج السبب الفعلي المدخل عند التعديل
    let reason = '';
    if (app.resolution_notes && String(app.resolution_notes).trim()) {
        reason = String(app.resolution_notes).trim();
    } else if (app.decision_notes && String(app.decision_notes).trim()) {
        reason = String(app.decision_notes).trim();
    } else if (app.notes && String(app.notes).trim()) {
        reason = String(app.notes).trim();
    } else if (app.reason && String(app.reason).trim()) {
        reason = String(app.reason).trim();
    } else if (app.status === 'completed') {
        reason = 'تم تعديل ورصد الدرجة بالكنترول';
    } else {
        reason = 'قيد المراجعة والتدقيق';
    }
...
document.addEventListener('DOMContentLoaded', () => {
    console.log('🚀 Initializing Grade Appeals Module v1.0.5...');
...
function renderAppealsTable(appeals) {
    ...
    tbody.innerHTML = appeals.map((app, idx) => {
        ...
        // الدرجة السابقة لفتح نافذة التعديل بدقة
        const prevMidForModal = app.old_midterm_grade ?? app.current_midterm_grade ?? 0;
        const prevFinForModal = app.old_final_grade   ?? app.current_final_grade   ?? 0;
        ...
        if (!canEditAppeals) {
            ...
            actionsHtml = `
                ...
                <button type="button" class="btn-act btn-act-edit ${disCls}" ${dis} onclick="openEditModal(${app.appeal_id},'${escapeJsString(app.student_name)}','${escapeJsString(app.course_code)} - ${escapeJsString(app.course_name)}',${prevMidForModal},${prevFinForModal})">
...
// ============================================================
// 🖨️ طباعة نموذج الطعن الفردي (مصحح ديناميكياً)
// ============================================================
...
function printAppealForm(appealId) {
    ...
    const details = getAppealGradeDetails(app);
...
<div class="ap-page">
    ...
    <!-- جدول المواد المطعون فيها ديناميكياً بحسب النصفي أو النهائي -->
    <table class="ap-table">
        <thead>
            <tr>
                <th style="width:7%">م</th>
                <th style="width:23%">اسم المادة</th>
                <th style="width:16%">نوع الطعن</th>
                <th style="width:16%">درجة ${details.gradeTypeLabel}<br>قبل الطعن</th>
                <th style="width:16%">درجة ${details.gradeTypeLabel}<br>بعد الطعن</th>
                <th style="width:22%">السبب / القرار</th>
            </tr>
        </thead>
        <tbody>
            <tr>
                ...
                <td style="font-weight:bold;">${escapeHtml(app.appeal_type_display || (details.isMidterm ? 'امتحان نصفي' : 'امتحان نهائي'))}</td>
                <td style="font-weight:bold;font-size:13px;">${escapeHtml(String(details.gradeBefore))}</td>
                <td class="after-grade" style="font-weight:900;font-size:13px;color:#15803d;">${app.status === 'completed' ? escapeHtml(String(details.gradeAfter)) : '—'}</td>
                <td class="reason-cell" style="font-weight:600;">${escapeHtml(details.reason)}</td>
            </tr>
    ...
    <div style="font-size:11px;margin-bottom:14px;">* تُعتمد نتيجة الطعن بناءً على مراجعة كراسة الإجابة ولجنة المراجعة المختصة.</div>
    ...
    <!-- قسم قرار اللجنة والأستاذ -->
    <div class="ap-prof-section">
        <div class="ap-prof-title">🗓️ قرار لجنة المراجعة وأستاذ المادة:</div>
        <div class="ap-prof-row">
            <div class="ap-prof-field">
                <div style="font-weight:bold;margin-bottom:4px;">الدرجة بعد المراجعة:</div>
                <div style="border-bottom:1px solid #000;min-width:120px;height:24px;font-weight:bold;font-size:13px;padding-right:6px;">
                    ${app.status === 'completed' ? escapeHtml(String(details.gradeAfter)) : ''}
                </div>
            </div>
            <div class="ap-prof-field" style="flex:2;">
                <div style="font-weight:bold;margin-bottom:4px;">سبب التعديل / القرار:</div>
                <div style="border-bottom:1px solid #000;min-width:200px;height:24px;font-weight:600;padding-right:6px;">
                    ${escapeHtml(details.reason)}
                </div>
            </div>
            <div class="ap-prof-field">
                <div style="font-weight:bold;margin-bottom:4px;">اعتماد الأستاذ / اللجنة:</div>
                <div style="border-bottom:1px solid #000;min-width:100px;height:24px;text-align:center;">
                    ${app.status === 'completed' ? 'معتمد ✓' : ''}
                </div>
            </div>
        </div>
    </div>

    <!-- توقيع منسق الدراسة والامتحانات المعتمد -->
    <div style="margin-top:auto;padding-top:20px;text-align:left;margin-left:20px;" data-official="exam_director">
        <span class="off-name" style="display:block;font-size:13.5px;font-weight:bold;min-height:1.3em;">د. فاطمة عمران الشريف</span>
        <span class="off-pos" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;">رئيس قسم الدراسة والامتحانات</span>
        <span style="display:block;font-size:12px;letter-spacing:1px;">التوقيع والاعتماد: ....................................</span>
    </div>
</div>`;
...
// ============================================================
// 🖨️ طباعة كشف الحصر (ملخص كل الطعون)
// ============================================================
    ...
    const rows = currentAppealsList.map((app, idx) => {
        ...
        return `
            <tr>
                ...
                <td style="font-family:monospace;font-weight:bold;">${escapeHtml(app.student_id)}</td>
                <td class="name-cell">${escapeHtml(app.course_name)}</td>
                <td style="font-weight:600;">${escapeHtml(app.appeal_type_display || details.gradeTypeLabel)}</td>
                <td style="font-weight:bold;">${escapeHtml(String(details.gradeBefore))}</td>
                <td style="font-weight:900;color:#15803d;">${app.status === 'completed' ? escapeHtml(String(details.gradeAfter)) : '—'}</td>
                <td class="name-cell">${escapeHtml(details.reason)}</td>
    ...
    <table class="sr-table">
        <thead>
            <tr>
                ...
                <th style="width:17%">المادة</th>
                <th style="width:11%">نوع الطعن</th>
                <th style="width:10%">الدرجة قبل</th>
                <th style="width:10%">الدرجة بعد</th>
                <th style="width:18%">السبب / القرار</th>
            </tr>
        </thead>
        <tbody>${rows}</tbody>
    </table>

    <div style="margin-top:auto;padding-top:24px;text-align:left;margin-left:20px;" data-official="exam_director">
        <span class="off-name" style="display:block;font-size:13.5px;font-weight:bold;min-height:1.3em;">د. فاطمة عمران الشريف</span>
        <span class="off-pos" style="display:block;font-size:12px;font-weight:600;margin-bottom:4px;">رئيس قسم الدراسة والامتحانات</span>
        <span style="display:block;font-size:12px;letter-spacing:1px;">التوقيع والاعتماد: ....................................</span>
    </div>
...
console.log('✅ grade_appeals.js v1.0.5 ready');